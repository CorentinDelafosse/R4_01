    ...
    <div class="row no-gutters">
        <div class="col-md-12">
            <div class="product-details mr-2">
                ...
                <?php
                    $monProduit = get_result("SELECT * FROM Produit WHERE id = ".$monPanier['id_produit']);
                ...
                ?>
                    <div class="d-flex justify-content-between align-items-center mt-3 p-2 items rounded">
                        ...
                        <div class="d-flex flex-row align-items-center"><span class="d-block">Quantité : <?=$monPanier['quantite'] ?>  </span><span class="d-block ml-5 font-weight-bold">, Prix : <?=$monProduit['prix']*$monPanier['quantite'] ?>€</span><i class="fa fa-trash-o ml-3 text-black-50"></i>

                            <form action="panier/<?=$monProduit['identifiant']?>/" method="post">

                                <input class="form-control text-center me-3" name="panier_quantite" type="number" value="1" min="1" max="<?=$monPanier['quantite'] ?>" style="max-width: 4em" oninput="this.value = Math.min(Math.max(parseInt(this.value), parseInt(this.min)), parseInt(this.max))"/>
